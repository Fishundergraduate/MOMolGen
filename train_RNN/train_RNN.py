...
if __name__ == "__main__":
    ...
    print(X.dtype)
    trainDataset = data.Dataset.from_tensor_slices((X_nd_train[...,tf.newaxis], y_nd_train_one_hot[...,tf.newaxis])).with_options(dataOpt)
    validDataset = data.Dataset.from_tensor_slices((X_nd_valid[...,tf.newaxis], y_nd_valid_one_hot[...,tf.newaxis])).with_options(dataOpt)
    ...
    earlystoppingByTimer = EarlyStoppingByTimer(timeLimit=datetime.timedelta(minutes=1))
