...
class Node(dict):
    ...
    def __init__(self, position = None,  parent = None, state = None, childNodes=[], child=None, wins=[0,0,0,0], visits=0, nonvisited_atom=None, type_node= [], depth=0):
        super().__init__()
        self.__dict__ = self
...
def MCTS(root, verbose = False, pareto=pareto(), time_limit_sec=3600*240):
    ...
    while time.time()<=run_time:
        ...
        f = open("./data/present/ligands.txt", 'w')
        for p in valid_smile:
            print(p,file=f)
        f.close()
        
        f = open("./data/present/scores.txt", 'w')
        for s in scores:
            print(s,file=f)
        f.close()
        """backpropation step"""
        ...
        pareto_file = open('./data/present/pareto.json', 'w')
        json.dump(pareto.__dict__,pareto_file, indent=4, separators=(',', ': '))
        pareto_file.close()

        mct_file = open('./data/present/tree.json', 'w')
...
if __name__ == "__main__":
    ...
    if os.path.exists('./ligand_design/config.json') :
        ...
        pareto_locate = config['whereisParetoFile']
